...
<div class="login__footer">
  ...
  public static function adjustCompanyFields(array $fields, $address_type) {
    ...
    if ($value === 'Firma') {
      ...
      $_POST[$address_type . '_company_contact'] = '';
    }
    return $fields;
  }
  ...
  public static function woocommerce_after_checkout_validation($data, $errors) {
    ...
    if (empty($missing_fields)) {
      ...
      if (isset($response['abono']) || isset($response['id'])) {
        ...
        // Do not output an error if the current user has the same subscriber ID
        // or attempts to associate their account with the returned subscriber ID,
        ...
        elseif (!empty($response['abono']) && empty($response['id'])) {
          ...
          // address (trial subscription).
          if (!$new_subscriber_data && get_user_meta(get_current_user_ID(), 'alfa_dummy_address', TRUE)) {
            $subscriber_data = [
  ...
  public static function woocommerce_checkout_order_processed($order_id, $posted_data, $order) {
    ...
    if (!static::$isAnonymousCheckout) {
      ...
      if ($subscriber = WC()->session->get('subscriber_data')) {
        ...
        if (get_user_meta($order->get_customer_id(), 'alfa_dummy_address', TRUE)) {
    ...
    if (!isset($response['statuscode']) || $response['statuscode'] !== 200) {
      ...
      if (!static::$isAnonymousCheckout) {
        wc_add_notice(vsprintf('Haben Sie digitale Zugänge erworben? Um diese zu aktivieren, <a href="%s">melden Sie sich bitte ab und erneut an</a>.', [
  ...
  public static function woocommerce_after_save_address_validation($user_id, $address_type, $address) {
    ...
    if ($subscriber = WC()->session->get('subscriber_data')) {
      ...
      if (get_user_meta($user_id, 'alfa_dummy_address', TRUE)) {
        Server::checkPurchase($purchase['id'], $purchase['subscriber_id']);
      }
    }
